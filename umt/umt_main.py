...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="--- Raspbery Pi Urban Mobility Tracker ---"
    )
    parser.add_argument(
        "-modelpath",
        dest="model_path",
    ...
    parser.add_argument(
        ...
        type=str,
        required=False,
        help="specify an image sequence",
    )
    parser.add_argument(
        "-video", dest="video_path", type=str, required=False, help="specify video file"
    )
    parser.add_argument(
        "-camera",
        dest="camera",
    ...
    if args.model_path:
        assert (
            os.path.exists(args.model_path) == True
        ), "can't find the specified model..."
    if args.label_map_path:
        assert (
            os.path.exists(args.label_map_path) == True
        ), "can't find the specified label map..."
    if args.video_path:
        assert (
            os.path.exists(args.video_path) == True
    ...
    if not os.path.exists("output") and args.save_frames:
        os.makedirs("output")
    ...
    with open(TRACKER_OUTPUT_TEXT_FILE, "w") as out_file:
        for i, pil_img in enumerate(img_generator(args)):
            ...
            if i == 0:
                header = (
                    f"frame_num,rpi_time,obj_class,obj_id,obj_age,"
            ...
            if args.live_view or args.save_frames:
                ...
                for track in tracker.tracks:
                    ...
                    cv2.rectangle(
                        ...
                        (
                            int(bbox[0])
                            + (len(str(class_name)) + len(str(track.track_id))) * 17,
                            int(bbox[1]),
                        ),
                        color,
                        -1,
                    ...
                    cv2.putText(
                        cv2_img,
                        str(class_name) + "-" + str(track.track_id),
